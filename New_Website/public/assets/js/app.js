...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadMoreBtn.addEventListener('click', function() {
        window.location.href = "TopManga";
    });
});
document.addEventListener('DOMContentLoaded', function() {
    let selectBox = document.getElementById('orderBySelect');
    ...
    RESHOP.initModal = function() {
        ...
        if ($collectionCartModalLink.length) {
            $collectionCartModalLink.on('click', function () {
                const getElemId = $(this).data('modal-id');
                // Recupera i dati necessari
                const prezzo = $(this).data('prezzo');
                const prezzoOriginale = $(this).data('prezzo-originale');
                const casaEditrice = $(this).data('casa-editrice');
                const nome = $(this).data('nome');
                const descrizione = $(this).data('descrizione');
                const prodottiDisponibili = $(this).data('prodotti-disponibili');
                const id = $(this).data('id');
                const ROOT = $(this).data('root');
                const imgSrc = $(this).data('img-src');

                console.log(prodottiDisponibili)
                document.getElementById('modal-add-to-cart-count').style.display = ""
                document.getElementById('modal-add-to-cart-button-si').style.display = ""
                document.getElementById('modal-add-to-cart-button-no').style.display = "none"
                document.getElementById('Modal-data-max').setAttribute('data-max', prodottiDisponibili);
                document.getElementById('Modal-data-max').value = 1;

                const href = ROOT+"cart"
                

                document.getElementById('id-prodotto').value = id

                document.getElementById('add-to-cart').action = href

                document.getElementById("modal-img-src").src = imgSrc

                var percentualeSconto = Math.round((( Number(prezzo) - Number(prezzoOriginale) ) / Number(prezzo)) * 100)


                if (prezzo !== 0){
                    document.getElementById('sconto-vedi').style.display = ""
                    document.getElementById('non-sconto-vedi').style.display = "none"
                }else {
                    document.getElementById('sconto-vedi').style.display = "none"
                    document.getElementById('non-sconto-vedi').style.display = ""
                }



                if(prodottiDisponibili>10 && prodottiDisponibili<100){



                    document.getElementById('quantita-grandi').style.display = ""
                    document.getElementById('quantita-piccole').style.display = "none"
                    document.getElementById('quantita-grandissime').style.display = "none"
                    document.getElementById('quantita-no').style.display = "none"
                    let text
                    $(getElemId).find('.modal-prodotti-disponibili-grandi').text(prodottiDisponibili+" in stock");


                }else if(prodottiDisponibili<=10 && prodottiDisponibili>0){



                    document.getElementById('quantita-grandi').style.display = "none"
                    document.getElementById('quantita-piccole').style.display = ""
                    document.getElementById('quantita-grandissime').style.display = "none"
                    document.getElementById('quantita-no').style.display = "none"
                    $(getElemId).find('.modal-prodotti-disponibili-piccole').text("solo "+prodottiDisponibili+" rimanenti");


                }else if(prodottiDisponibili<=0){

                    document.getElementById('quantita-grandi').style.display = "none"
                    document.getElementById('quantita-piccole').style.display = "none"
                    document.getElementById('quantita-grandissime').style.display = "none"
                    document.getElementById('modal-add-to-cart-count').style.display = "none"
                    document.getElementById('quantita-no').style.display = ""
                    document.getElementById('modal-add-to-cart-button-si').style.display = "none"
                    document.getElementById('modal-add-to-cart-button-no').style.display = ""
                    ...
                    document.getElementById('quantita-grandi').style.display = "none"
                    document.getElementById('quantita-piccole').style.display = "none"
                    document.getElementById('quantita-grandissime').style.display = ""
                    document.getElementById('quantita-no').style.display = "none"
                ...
                $(getElemId).find('.modal-prezzo').text(prezzo+"€")
                $(getElemId).find('.modal-prezzo-originale').text(prezzoOriginale+"€");
                $(getElemId).find('.modal-casa-editrice').text(casaEditrice);
                $(getElemId).find('.modal-nome').text(nome);
                $(getElemId).find('.modal-descrizione').text(descrizione);
                $(getElemId).find('.modal-id').text(id);
                $(getElemId).find('.modal-sconto').text(percentualeSconto+"%");


                $(getElemId).find('.modal-prezzo').show = false
